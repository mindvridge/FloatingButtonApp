...
fun ServiceControlScreen(
    ...
    // 화면 진입 시 권한이 모두 있고 서비스가 실행중이지 않으면 자동으로 시작 (사용자가 중지하지 않은 경우만)
    LaunchedEffect(hasOverlayPermission, hasAccessibilityPermission) {
        if (hasOverlayPermission && hasAccessibilityPermission && !isServiceRunning && !userStoppedService) {
            Log.d("PermissionScreens", "권한 완료 - 서비스 자동 시작")
            onStartServiceClick()
        }
    }
